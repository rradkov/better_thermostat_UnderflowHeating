...
**Heat-source flow/water temperature sensor** and **...static fallback (°C)** — both optional, and only relevant if you use the **(AI) MPC v2** calibration mode (see the second step below). MPC v2's plant model assumes a 65°C heat source by default — a reasonable prior for a gas/oil boiler, but off by a wide margin for a heat pump or any weather-compensated system, which typically runs 30–45°C. If you have a sensor tracking your actual flow/water temperature (e.g. into a buffer tank, or your heat pump's own flow-temp sensor), point this at it — it's read live on every control cycle, tracking weather compensation through the season with no extra setup. The static fallback covers setups without such a sensor (or backs up a sensor that's occasionally unavailable); leave both blank to keep the 65°C default unchanged. Every other calibration mode ignores both fields entirely — none of them assume a fixed heat-source temperature the way MPC v2's physical model does.
